...
| 2026-08-16 | **Implemented & verified.** Deviations during build: (1) removed `pytest` version pin — clashed with the Airflow constraint file; (2) pinned `dbt-core==1.9.11` / `dbt-postgres==1.9.1` because `dbt-postgres` pulled the dbt 2.0 "Fusion" pre-release which dropped Postgres support ([ADR 0003](../docs/adr/0003-pin-classic-dbt-engine.md)); (3) installed `git` in the image (required by dbt / `dbt debug`). Also used **two** Postgres services (warehouse + Airflow metadata) for separation of concerns. All acceptance criteria passed: services healthy, schemas created, `dbt debug` green, `pytest` 2/2, skeleton DAG run = success. |
| 2026-08-16 | Bake `dags/` and `dbt/` into the image (self-contained artifact) with compose bind mounts overriding for local dev; added `.dockerignore` ([ADR 0004](../docs/adr/0004-project-code-baked-into-image.md)). Verified: image lists baked files with no mounts; `dbt debug` still green. |
