...
| 2026-08-19 | **Quarantine-first extension.** Added plausibility/grain reasons (`price_non_positive`, `tax_negative`, `quantity_non_positive`, `duplicate_transaction_id`) in `stg_customer_transactions`, so detectable-but-unexpected problems are quarantined (run stays green) instead of hard-failing dbt tests — we decide later if serious; tests remain a backstop. Widened `dim_date` spine (2020–2031). Verified by injecting a duplicate id + zero qty + zero price → all quarantined, run green (fact 71→70, quar 29→33, reconciled), then restored. See [ADR 0005](../docs/adr/0005-data-quality-strategy.md). |
