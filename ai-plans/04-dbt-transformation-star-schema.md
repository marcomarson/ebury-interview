...
| 2026-08-17 | **Aggregates enriched** (brief §2 "consider what to include"): monthly + `avg_transaction_value`, `avg_units_per_transaction`, `distinct_products`; by-customer + `avg_transaction_value`, `distinct_products`, `first/last_transaction_date`, `is_unknown`; by-product + `avg_unit_price`, `revenue_share_pct`. Verified (shares sum to 100%). |
